...
<xsl:template match='/aiadm:ADMIN_PERSON_LIST'>
<rdf:RDF>
<xsl:for-each select='aiadm:PERSON'>

<xsl:variable name='ctr'  select='@counter'/>
<xsl:variable name='uno' select='$unomap/map[position()=$ctr]/@nuno'/>

<xsl:variable name='knownUri' 
select='vfx:knownUri(aiadm:fname, aiadm:mname, aiadm:lname, $extantPersons)'/>

<xsl:variable name='peruri' 
select="if( != '') then  else concat($g_instance,)"/>
...
<core:middleName><xsl:value-of select='aiadm:mname'/></core:middleName>
<core:firstName><xsl:value-of select='aiadm:fname'/></core:firstName>
<foaf:firstName><xsl:value-of select='aiadm:fname'/></foaf:firstName>
<core:lastName><xsl:value-of select='aiadm:lname'/></core:lastName>
<foaf:lastName><xsl:value-of select='aiadm:lname'/></foaf:lastName>
...
<xsl:with-param name='list' select='aiadm:ADMIN_LIST'/>
...
<xsl:variable name='peruri' 
select="if($knownUri != '') then  else concat($g_instance,$uno)"/>
...
<xsl:value-of select='aiadm:fname'/>
</xsl:element>
<xsl:element name='mname' namespace=''>
<xsl:value-of select='aiadm:mname'/>
</xsl:element>
<xsl:element name='lname' namespace=''>
<xsl:value-of select='aiadm:lname'/>
</xsl:element>
</xsl:element>
</xsl:if>
...
<xsl:for-each select='$list/aiadm:ADMIN'>
<!-- =================================================== -->
<!-- Declare property mapping ???Admin to foaf:Person -->
<!-- 1 -->
<xsl:variable name='objid' select='@id'/>
<rdf:Description rdf:about="{concat($g_instance,'AI-',$objid)}" >
<acti:adminOf rdf:resource="{$objref}"/>
</rdf:Description>
<!-- =================================================== -->
<!-- Declare property mapping foaf:Person to ???Admin -->
<!-- 2 -->
