...
<xsl:variable name='x' select='$all_aiid_netid[mapid:netid = $cur/netid][1]'/>

<!-- xsl:comment><xsl:value-of select='$cur'/> <xsl:value-of select='$x'/></xsl:comment-->
<xsl:if test='$x'>
...
<foaf:firstName><xsl:value-of select='vfx:simple-trim(fname)'/></foaf:firstName>
<core:middleName><xsl:value-of select='vfx:simple-trim(mname)'/></core:middleName>
<foaf:lastName><xsl:value-of select='vfx:simple-trim(lname)'/></foaf:lastName>
</rdf:Description>
</xsl:if>
