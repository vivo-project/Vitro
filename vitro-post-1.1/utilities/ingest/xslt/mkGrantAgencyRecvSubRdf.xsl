...
<xsl:template match='/aigrant:AGENCIES_RECEIVING_SUBMISSIONS'>
<rdf:RDF>
<xsl:for-each select='aigrant:AGENCY'>
...
select='vfx:knownOrgUri(vfx:trim(aigrant:AGENCY_NAME), $extantOrgs)'/>
...
<rdfs:label>
<xsl:value-of select='vfx:trim(aigrant:AGENCY_NAME)'/>
</rdfs:label>
</rdf:Description>
</xsl:if>
