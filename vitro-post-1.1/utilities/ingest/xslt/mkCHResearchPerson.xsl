...
<xsl:stylesheet version='2.0'
	...
	exclude-result-prefixes='xs vfx dm aichr'
	>
...
<!-- ================================== -->
<xsl:template match='/aichr:CHRESEARCH_PERSON_LIST'>
<rdf:RDF>

<xsl:for-each select='aichr:CHRESEARCH_BY_PERSON'>
...
select='vfx:knownUri(aichr:FirstName, 
aichr:MiddleName, aichr:LastName, $extantPersons)'/>
...
<xsl:if test='$knownUri != "" and aichr:NetId != ""'>
...
<rdfs:label>
<xsl:value-of select='vfx:trim(aichr:PERSON_NAME)'/>
</rdfs:label>

<core:middleName><xsl:value-of select='aichr:MiddleName'/></core:middleName>
<core:firstName><xsl:value-of select='aichr:FirstName'/></core:firstName>
<foaf:firstName><xsl:value-of select='aichr:FirstName'/></foaf:firstName>
<core:lastName><xsl:value-of select='aichr:LastName'/></core:lastName>
<foaf:lastName><xsl:value-of select='aichr:LastName'/></foaf:lastName>
...
<!-- =================================================== -->
<!-- now process the CHRESEARCHs attributed to this person -->

<xsl:call-template name='process-person'>
<xsl:with-param name='objbyi' select='aichr:CHRESEARCH_LIST'/>
...
<xsl:value-of select='aichr:FirstName'/>
</xsl:element>
<xsl:element name='mname' namespace=''>
<xsl:value-of select='aichr:MiddleName'/>
</xsl:element>
<xsl:element name='lname' namespace=''>
<xsl:value-of select='aichr:LastName'/>
</xsl:element>
</xsl:element>
</xsl:if>
...
<xsl:for-each select='$objbyi/aichr:CHRESEARCH_INFO'>
<xsl:variable name='aiid' select='.'/>
<xsl:variable name='rank' select='@collabRank'/>
<xsl:variable name='role' select='../../aichr:Role'/>
<!-- =================================================== -->
<!-- Declare property mapping CHRESEARCH to core:ResearcherRole -->

<rdf:Description rdf:about="{concat($g_instance,$aiid)}" >
<!-- 1 -->
<core:relatedRole
	rdf:resource="{concat($g_instance,$aiid,'-CHR-',$rank)}"/>
</rdf:Description>

<!-- =================================================== -->
<!-- Declare core:ResearcherRole Individual Triples-->

<rdf:Description rdf:about="{concat($g_instance,$aiid,'-CHR-',$rank)}">

<rdfs:label>
<xsl:value-of select='concat( "Researcher (",vfx:trim(../../aichr:PERSON_NAME), ")")'/>
...
<!-- 4 -->
<core:hasResearcherRole rdf:resource="{concat($g_instance,$aiid,'-CHR-',$rank)}"/>
</rdf:Description>
</xsl:if>
</xsl:for-each>
