...
<xsl:element name="aiis:CONAREA_LIST" 
	namespace="http://vivoweb.org/ontology/activity-insight">

<!-- =============== -->
<!-- 
 gather all CONAREA_PLACE sub-xmls for a given name and sort group collection
 by uppercased constructed name
-->
<xsl:for-each-group select='$docs/Data/CONAREA_LIST/CONAREA_INFO' 
	group-by='vfx:collapse(CONAREA_NAME)'>
<xsl:sort select='vfx:collapse(CONAREA_NAME)'/>
...
<!--
   for the given CONAREA_INFO group ( . is the first in each group)

-->

   <xsl:element name='aiis:CONAREA_NAME'>
	...
	<xsl:value-of select='@ilk'/></xsl:attribute>
     <xsl:value-of select='vfx:trim(CONAREA_NAME)'/>
