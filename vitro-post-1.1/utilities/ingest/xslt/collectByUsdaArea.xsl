...
<xsl:element name="aiis:USDA_LIST" 
	namespace="http://vivoweb.org/ontology/activity-insight">

<!-- =============== -->
<!-- 
 gather all USDA sub-xmls for a given name and sort group collection
 by uppercased constructed name
-->
<xsl:for-each-group select='$docs/Data/USDA_LIST/USDA_AREA_INFO' 
	group-by='vfx:collapse(USDA_AREA_NAME)'>
<xsl:sort select='vfx:collapse(USDA_AREA_NAME)'/>
...
<xsl:element name="aiis:IMPACT_STMTS_BY_USDA_AREA" >
 <xsl:attribute name='ilk'>
       <xsl:value-of select='@ilk'/></xsl:attribute>
<!-- =============== -->

<!--
   for the given USDA_AREA group ( . is the first in each group)

-->

   <xsl:element name='aiis:USDA_AREA_NAME'>
     <xsl:attribute name='netid_count'>
       <xsl:value-of select='count(current-group())'/></xsl:attribute>
  
     <xsl:value-of select='vfx:trim(USDA_AREA_NAME)'/>
