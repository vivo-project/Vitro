...
public abstract class BaseIndividualTemplateModel extends BaseTemplateModel {
    ...
    public BaseIndividualTemplateModel(Individual individual, VitroRequest vreq) {
        ...
        // Needed for getting portal-sensitive urls. Remove if multi-portal support is
        // removed.
    ...
    /**
     * this method get the individual properties list and fetch the object
    ...
    /**
     * This page is editable if the user is authorized to add a data property or an
     * object property to the Individual being shown.
     */
    public boolean isEditable() {
        AddDataPropertyStatement adps = new AddDataPropertyStatement(vreq.getJenaOntModel(), individual.getURI(),
                SOME_URI, SOME_LITERAL);
        AddObjectPropertyStatement aops = new AddObjectPropertyStatement(vreq.getJenaOntModel(), individual.getURI(),
                SOME_PREDICATE, SOME_URI);
        return PolicyHelper.isAuthorizedForActions(vreq, adps.or(aops));
    }

    public boolean getShowAdminPanel() {
        return PolicyHelper.isAuthorizedForActions(vreq, SimplePermission.SEE_INDVIDUAL_EDITING_PANEL.ACTION);
    }

    /*
     * rdfs:label needs special treatment, because it is not possible to construct a
     * DataProperty from it. It cannot be handled the way the vitro links and vitro
     * public image are handled like ordinary ObjectProperty instances.
     */
    ...
    /*
     * These methods simply forward to the methods of the wrapped individual. It
     * would be desirable to implement a scheme for proxying or delegation so that
     * the methods don't need to be simply listed here. A Ruby-style method missing
     * method would be ideal. Update: DynamicProxy doesn't work because the proxied
     * object is of type Individual, so we cannot declare new methods here that are
     * not declared in the Individual interface.
     */
    ...
    public String selfEditingId() {
        String id = null;
        String idMatchingProperty = ConfigurationProperties.getBean(ctx).getProperty("selfEditing.idMatchingProperty");
        if (!StringUtils.isBlank(idMatchingProperty)) {
            WebappDaoFactory wdf = vreq.getUnfilteredWebappDaoFactory();
            Collection<DataPropertyStatement> ids = wdf.getDataPropertyStatementDao()
                    .getDataPropertyStatementsForIndividualByDataPropertyURI(individual, idMatchingProperty);
            if (ids.size() > 0) {
