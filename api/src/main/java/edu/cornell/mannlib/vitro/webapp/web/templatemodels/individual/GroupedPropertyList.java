...
public class GroupedPropertyList extends BaseTemplateModel {
    ...
    GroupedPropertyList(Individual subject, VitroRequest vreq, boolean editing) throws Exception {
        this.vreq = vreq;
        ...
        Map<String, List<String>> populatedObjTypes = makePopulatedObjTypeMap(populatedObjectPropertyList);
        ...
        Collection<ObjectProperty> additions = ApplicationConfigurationOntologyUtils
                .getAdditionalFauxSubpropertiesForList(populatedObjectPropertyList, subject, vreq);
        ...
        if (log.isDebugEnabled()) {
            ...
            log.debug("Added " + additions.size() + " properties due to application configuration ontology");
        }
        ...
        // If editing this page, merge in object properties applicable to the individual
        // that are currently
        // unpopulated, so the properties are displayed to allow statements to be added
        // to these properties.
        // RY In future, we should limit this to properties that the user has permission
        // to add properties to.
        if (editing) {
            propertyList = mergeAllPossibleObjectProperties(populatedObjectPropertyList, propertyList);
        }

        // Now do much the same with data properties: get the list of populated data
        // properties, then add in placeholders for missing ones
        // rjy7 Currently we are getting the list of properties in one sparql query,
        // then doing a separate query
        // to get values for each property. This could be optimized by doing a single
        // query to get a map of properties to
        // DataPropertyStatements. Note that this does not apply to object properties,
        // because the queries
        // can be customized and thus differ from property to property. So it's easier
        // for now to keep the
        ...
        for (PropertyGroup propertyGroup : propertyGroupList) {
            PropertyGroupTemplateModel pgtm = new PropertyGroupTemplateModel(vreq, propertyGroup, subject, editing,
        ...
        for (PropertyGroupTemplateModel pgtm : groups) {
            ...
            for (Future<?> f : futures) {
                ...
                try {
                    ...
                    } else if (o instanceof DataPropertyTemplateModel) {
                        DataPropertyTemplateModel dptm = (DataPropertyTemplateModel) o;
                        properties.add(dptm);
                    } else {
                        log.error("unexpected class " + o.getClass());
                    }
    ...
    // It's possible that an object property retrieved in the call to
    // getPopulatedObjectPropertyList()
    // is now empty of statements, because if not editing, some statements without a
    // linked individual
    // are not retrieved by the query. (See <linked-individual-required> elements in
    // queries.)
    // Remove these properties, and also remove any groups with no remaining
    // properties.
    ...
    private List<Property> mergeAllPossibleObjectProperties(List<ObjectProperty> populatedObjectPropertyList, List<Property> propertyList) {

        // There is no ObjectPropertyDao.getAllPossibleObjectPropertiesForIndividual()
        // parallel to
        // DataPropertyDao.getAllPossibleDatapropsForIndividual(). The comparable method
        ...
        // breaks blank node structures in the restrictions that determine applicable
        // properties.
        ...
        // These properties are outside the ontologies (in vitro and vitro public) but
        // need to be added to the list.
        // In future, vitro ns props will be phased out. Vitro public properties should
        // be changed so they do not
        // constitute a special case (i.e., included in
        // piDao.getAllPossiblePropInstForIndividual()).
    ...
    private ObjectProperty assembleObjectProperty(PropertyInstance pi) {
        WebappDaoFactory rawWadf = ModelAccess.on(vreq).getWebappDaoFactory(LANGUAGE_NEUTRAL, POLICY_NEUTRAL);
    ...
    /**
     * Don't know what the real problem is with VIVO-976, but somehow we have the
     * same property showing up once with a blank node as a domain, and once with
     * null or OWL:Thing as a domain.
     *
     * Similarly, don't know the real problem with VIVO-989, except that the ranges
     * are both blank nodes - probably the same blank node but on two different
     * reads.
     *
     * For VIVO-1015, if op2 (the unpopulated property) is a Faux property, it will
     * appear to be not redundant because of the range difference, and that's what
     * we want. But if op2 (the unpopulated property) has a different range than op1
     * because of a restriction, then we want to ignore that difference, so it
     * appears to be redundant.
     */
    private boolean redundant(ObjectProperty populatedOP, ObjectProperty possibleOP) {
        if (new FullPropertyKey((Property) populatedOP).equals(new FullPropertyKey((Property) possibleOP))) {
            return true;
        } else if (new FullPropertyKey(fudgeBlankNodeInDomain(populatedOP.getDomainVClassURI()), populatedOP.getURI(),
                populatedOP.getRangeVClassURI())
                        .equals(new FullPropertyKey(fudgeBlankNodeInDomain(possibleOP.getDomainVClassURI()),
                                possibleOP.getURI(), possibleOP.getRangeVClassURI()))) {
            return true;
        } else if (new FullPropertyKey(populatedOP.getDomainVClassURI(), populatedOP.getURI(),
                fudgeBlankNodeInRange(populatedOP.getRangeVClassURI()))
                        .equals(new FullPropertyKey(possibleOP.getDomainVClassURI(), possibleOP.getURI(),
    ...
    private List<PropertyGroup> addPropertiesToGroups(List<Property> propertyList) {
        ...
        /*
         * If no property groups are defined, create a dummy group with a null name to
         * signal to the template that it's not a real group. This allows the looping
         * structure in the template to be the same whether there are groups or not.
         */
        if (groupCount == 0) {
        ...
        /*
         * This group will hold properties that are not assigned to any groups. In case
         * no real property groups are populated, we end up with the dummy group case
         * above, and we will change the name to null to signal to the template that it
         * shouldn't be treated like a group.
         */
        ...
        try {
            ...
            log.error("Exception on trying to prune unpopulated groups from group list: " + ex.getMessage());
        }
    ...
    public PropertyTemplateModel pullProperty(String propertyUri, String rangeUri) {
        ...
        for (PropertyGroupTemplateModel pgtm : groups) {
            List<PropertyTemplateModel> properties = pgtm.getProperties();
            for (PropertyTemplateModel ptm : properties) {
                if (propertyUri.equals(ptm.getUri()) && (rangeUri == null || rangeUri.equals(ptm.getRangeUri()))) {
