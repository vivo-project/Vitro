...
/**
 * Access object for interacting with DataDistributor and GraphBuilder
 * configurations in the Display model
 */
public class DataDistributorDaoJena extends JenaBaseDao implements DataDistributorDao {
    ...
    private static final String ACTION_NAME_URI =
            "http://vitro.mannlib.cornell.edu/ns/vitro/ApplicationSetup#actionName";
    private static final String BUILDER_NAME_URI =
            "http://vitro.mannlib.cornell.edu/ns/vitro/ApplicationSetup#builderName";
    ...
    private static final String ALL_GRAPHBUILDERS = ""
            ...
            + "SELECT ?graphbuilder  \n" //
            + "WHERE { \n" //
            + "   ?graphbuilder a <java:edu.cornell.library.scholars.webapp."
            + "controller.api.distribute.rdf.graphbuilder.GraphBuilder> . \n" //
            + "} \n";
    ...
    public List<String> getDistributorUris() {
        ...
        return createSelectQueryContext(displayModel, ALL_DISTRIBUTORS).execute().toStringFields("distributor")
                .flatten();
    }
    ...
    public List<String> getGraphBuilderUris() {
        OntModel displayModel = getWebappDaoFactory().getOntModelSelector().getDisplayModel();
        return createSelectQueryContext(displayModel, ALL_GRAPHBUILDERS).execute().toStringFields("graphbuilder")
                .flatten();
    }
    ...
    public String getNameFromModel(Model model) {
        String nameUri = null;
        // Determine which Property we need to access depending on whether it is a
        // DataDistributor or GraphBuilder
    ...
    public Class getClassFromModel(Model model) {
        ...
        while (typeIterator.hasNext()) {
            Statement statement = typeIterator.nextStatement();
            if (objectClass == null) {
                // Only use the class if it is the "most significant" (not the base
                // DataDistributor or GraphBuilder interface)
    ...
    /**
     * Retrieve the significant (not DataDistributor or GraphBuilder interface)
     * class of the object
     */
