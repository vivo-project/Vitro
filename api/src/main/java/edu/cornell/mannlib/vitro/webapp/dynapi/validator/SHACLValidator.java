...
public class SHACLValidator implements ModelValidator {
    ...
    private final String queryText = "PREFIX rdf:      <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
...
//        }
    ...
    private Resource validateFileOverSHACLRules(String path, String format) throws IOException {
        Model dataModel = JenaUtil.createMemoryModel();
    ...
    public boolean isValidFile(String path, String format){
        Resource report = null;
        try {
            report = validateFileOverSHACLRules(path, format);
        } catch (IOException e) {
