...
public abstract class AbstractOntModelDecorator implements OntModel {
    ...
    /**
     * Initialize, setting the passed model as the inner model.
     *
     * @param m The internal model to be delegated.
     */
    ...
    // ==== These methods are directly on OntModel.class. ====
    ...
    public MaxCardinalityQRestriction createMaxCardinalityQRestriction(String uri, Property prop, int cardinality, OntClass cls) {
        return inner.createMaxCardinalityQRestriction(uri, prop, cardinality, cls);
    }

    @Override
    public MinCardinalityQRestriction createMinCardinalityQRestriction(String uri, Property prop, int cardinality, OntClass cls) {
        return inner.createMinCardinalityQRestriction(uri, prop, cardinality, cls);
    }
    ...
    public Model writeAll(Writer writer, String lang) {
        return inner.writeAll(writer, lang);
    }

    @Override
    public Model writeAll(OutputStream out, String lang) {
        return inner.writeAll(out, lang);
    }

    // ==== These methods are directly on InfModel.class. ====
    ...
    // ==== These methods are directly on Model.class. ====
    ...
    public Statement getRequiredProperty(Resource s, Property p, String lang) {
        return inner.getRequiredProperty(s, p, lang);
    }
    ...
    public Statement getProperty(Resource s, Property p, String lang) {
        return inner.getProperty(s, p, lang);
    }
    ...
    public void executeInTxn(Runnable action) {
        inner.executeInTxn(action);
    }

    @Override
    public <T> T calculateInTxn(Supplier<T> action) {
        return inner.calculateInTxn(action);
    }
    ...
    public RDFList getList(String uri) {
        return inner.getList(uri);
    }

    @Override
    public RDFList getList(Resource r) {
        return inner.getList(r);
    }
    ...
    @Override
    public StmtIterator listLiteralStatements(Resource subject, Property predicate,
            boolean object) {
    ...
    // ==== These methods are directly on ModelGraphInterface.class. ====
    ...
    // ==== These methods are directly on RDFReaderF.class. ====
    ...
    // ==== These methods are directly on RDFWriterF.class. ====
    ...
    // ==== These methods are directly on PrefixMapping.class, discluding any overridden by OntModel.class. ====
    ...
    // ==== These methods are directly on Lock.class. ====
