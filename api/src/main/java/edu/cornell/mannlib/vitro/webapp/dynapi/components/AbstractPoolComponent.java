...
public abstract class AbstractPoolComponent implements AutoCloseable {
    ...
    public void addClient() {
        ...
        if (oldName == null) {
            ...
            log.debug(String.format("Client readded to %s, client id %s. Old client name %s is replaced by %s", uri, id, oldName ,clientName));
        }
    }
    ...
    public List<String> getClients(){
       return clientIds.entrySet().stream().map(e -> String.valueOf(e.getKey()) + ":" + e.getValue()).collect(Collectors.toList());
    }
