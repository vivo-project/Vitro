...
public class UserAccessWhitelist implements AccessWhitelist {
	...
	public boolean isAuthorized(UserAccount user) {
		...
		if (users.contains(email)) {
			log.debug("User '" + user.getEmailAddress() + "' allowed to access action '" + actionName + "'");
			return true;
		}
		log.debug("User acccess whitelist is doesn't allow user '" + user.getEmailAddress() + "' to access action '" + actionName + "'");
		return false;
	}
