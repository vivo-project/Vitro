...
public class DrillDownGraphBuilder extends AbstractSparqlBindingGraphBuilder {
    private static final Log log = LogFactory.getLog(DrillDownGraphBuilder.class);
    ...
    @Property(uri = "http://vitro.mannlib.cornell.edu/ns/vitro/ApplicationSetup#topLevelGraphBuilder",
            minOccurs = 1)
    public void addTopLevelGraphBuilder(GraphBuilder builder) {
        topGraphBuilders.add(builder);
    }
    @Property(uri = "http://vitro.mannlib.cornell.edu/ns/vitro/ApplicationSetup#childGraphBuilder",
            minOccurs = 1)
    public void addChildGraphBuilder(GraphBuilder builder) {
        childGraphBuilders.add(builder);
    }
    @Property(uri = "http://vitro.mannlib.cornell.edu/ns/vitro/ApplicationSetup#drillDownQuery", minOccurs = 1,
            maxOccurs = 1)
    ...
    public Model buildGraph(DataDistributorContext ddContext) throws DataDistributorException {
        ...
        List<Map<String, String>> valueMaps = findDrillDownValues(topGraph, ddContext);
        log.debug("Results from SELECT: " + valueMaps);
        Model childBuildersGraph = runChildBuildersWithDrillDownValues(ddContext, valueMaps);
        return topGraph.add(childBuildersGraph);
    }
    private List<Map<String, String>> findDrillDownValues(Model topGraph, DataDistributorContext ddContext)
            throws DataDistributorException {
        QueryHolder query = bindParametersToQuery(ddContext, new QueryHolder(drillDownQuery));
        log.debug("binding the query\n   " + drillDownQuery + " becomes " + query.getQueryString());
        return createSelectQueryContext(topGraph, query).execute().toStringFields().getListOfMaps();
    }
    private Model runChildBuildersWithDrillDownValues(DataDistributorContext ddContext,
            List<Map<String, String>> valueMaps) throws DataDistributorException {
    ...
    private Model runChildBuilders(DataDistributorContext ddContext, Map<String, String> parameterSet)
            throws DataDistributorException {
        return new GraphBuilders(enhanceContext(ddContext, parameterSet), childGraphBuilders).run();
    }
    private EnhancedDataDistributionContext enhanceContext(DataDistributorContext ddContext,
            Map<String, String> parameterSet) {
        return new EnhancedDataDistributionContext(ddContext).addParameterValues(parameterSet);
    }
}
