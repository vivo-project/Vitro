 ...
 */
public class ConstructQueryGraphBuilder extends AbstractSparqlBindingGraphBuilder {
    private static final Log log = LogFactory.getLog(ConstructQueryGraphBuilder.class);
    ...
    public Model buildGraph(DataDistributorContext ddContext) throws DataDistributorException {
        ...
        for (String rawQuery : rawQueries) {
            QueryHolder query = bindParametersToQuery(ddContext, new QueryHolder(rawQuery));
            log.debug("Query is: " + query);

            m.add(createConstructQueryContext(rdfService, query).execute().toModel());
        }
        return m;
    }
}
