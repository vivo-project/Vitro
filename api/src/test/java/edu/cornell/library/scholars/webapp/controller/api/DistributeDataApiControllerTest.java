...
public class DistributeDataApiControllerTest extends AbstractTestClass {
    ...
    private static final String ACTION_NAME_PROPERTY =
            "http://vitro.mannlib.cornell.edu/ns/vitro/ApplicationSetup#actionName";
    private static final String FAIL_METHOD_PROPERTY =
            "http://vitro.mannlib.cornell.edu/ns/vitro/ApplicationSetup#failMethod";
    ...
    public void emptyAction_400_badRequest() throws ServletException, IOException {
        setActionPath("");
        populateDisplayModel(dd(DDIST_URI_1, DDTestDistributor.class, "simpleSuccess"));
    ...
    public void multipleActions_warning() throws ServletException, IOException {
        ...
        populateDisplayModel(dd(DDIST_URI_1, DDTestDistributor.class, "multiples"), dd(DDIST_URI_2,
                DDTestDistributor.class, "multiples"));
        runIt(200, "success");

        assertTrue(w.toString().contains("more than one DataDistributor for 'multiples'"));
    }
    ...
    public void failToInstantiate_500_serverError() throws ServletException, IOException {
        ...
        populateDisplayModel(dd(DDIST_URI_1, TestFailure.class, "simpleSuccess"));
    ...
    private Model dd(String uri, Class<?> class1, String actionName, String... failMethods) {
        Model m = model(typeStatement(uri, toJavaUri(class1)), typeStatement(uri, toJavaUri(DataDistributor.class)),
    ...
    private void runIt(int expectedStatus, String expectedOutput) throws ServletException, IOException {
        controller.doGet(req, resp);
        assertEquals(expectedStatus, resp.getStatus());
        String actualOutput = resp.getOutput();
        if (!actualOutput.contains(expectedOutput)) {
            fail("expect output to contain>" + expectedOutput + "<but was>" + actualOutput + "<");
        }
    }
    ...
    public static class DDTestDistributor implements DataDistributor {
        ...
        @SuppressWarnings("unused") @Property(uri = ACTION_NAME_PROPERTY)
        ...
        @Override
        public void init(DataDistributorContext ddContext) throws DataDistributorException {
        ...
        public String getContentType() throws DataDistributorException {
            if (failureMethods.contains("getContentType")) {
                throw new DataDistributorException("forced error in getContentType()");
            }
            return "text/plain";
        }
        ...
        public void writeOutput(OutputStream output) throws DataDistributorException {
            if (failureMethods.contains("writeOutput")) {
                throw new DataDistributorException("forced error in writeOutput()");
            }
            try {
