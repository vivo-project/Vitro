...
public class SparqlConstructQueryTest {
    ...
    private static final String QUERY_NO_VARS =
            "CONSTRUCT { <test:uri> <test:property> ?str . } WHERE { BIND(\"test\" as ?str) } ";
    private static final String QUERY_OBJ_VAR =
            "CONSTRUCT { ?s <test:property> \"bob\" . }  WHERE { ?s <test:property> ?str . } ";
    private static final String QUERY_SUBJ_VAR =
            "CONSTRUCT { <test:uri> <test:property> ?o . } WHERE { ?str <test:property> ?o . } ";
    private static final String QUERY_LABEL = "" +
            "CONSTRUCT { <test:uri> <http://www.w3.org/2000/01/rdf-schema#label> ?o . } " +
            "WHERE { ?str <http://www.w3.org/2000/01/rdf-schema#label> ?o . } ";
    ...
    public void emptyConfiguration() throws Exception {
        ...
        if (dataStore.contains(OUT_MODEL)) {
            ...
            assertTrue(outModel.getGraph().contains(NodeFactory.createURI("test:uri"), NodeFactory.createURI(
                    "test:property"), NodeFactory.createLiteral("test")));
        }
    }
    ...
    public void stringliteralInput() throws Exception {
        ...
        if (dataStore.contains(OUT_MODEL)) {
            Model outModel = (Model) TestView.getObject(dataStore.getData(OUT_MODEL));
            assertTrue(outModel.size() == 1);
            assertTrue(outModel.getGraph().contains(NodeFactory.createURI("test:resource"), NodeFactory.createURI(
                    "test:property"), NodeFactory.createLiteral("bob")));
        }
    }
    ...
    public void uriInput() throws Exception {
        ...
        if (dataStore.contains(OUT_MODEL)) {
            ...
            assertTrue(outModel.size() == 1);
            assertTrue(outModel.getGraph().contains(NodeFactory.createURI("test:uri"), NodeFactory.createURI(
                    "test:property"), NodeFactory.createLiteral("alice")));
        }
    }
    ...
    public void languageFiltering() throws Exception {
        ...
        if (dataStore.contains(OUT_MODEL)) {
            ...
            assertTrue(outModel.size() == 1);
            assertTrue(outModel.getGraph().contains(NodeFactory.createURI("test:uri"), NodeFactory.createURI(
                    "http://www.w3.org/2000/01/rdf-schema#label"), NodeFactory.createLiteral("Alicia", "es")));
        }
    }
