...
public class SparqlSelectQueryTest {
    ...
    private static final String QUERY_LIMIT_ORDER =
            "SELECT ?limit_o WHERE { ?str <test:property> ?limit_o . } ORDER BY ?order(?limit_o) LIMIT ?limit ";
    private static final String QUERY_LABEL =
            "SELECT ?o WHERE { ?str <http://www.w3.org/2000/01/rdf-schema#label> ?o . } ";
    ...
    public void languageFiltering() throws Exception {
        ...
        ParameterUtils.addStatement(model, "test:resource", "http://www.w3.org/2000/01/rdf-schema#label",
                "Alice@en-US");
        ParameterUtils.addStatement(model, "test:resource", "http://www.w3.org/2000/01/rdf-schema#label",
                "Алиса@ru-RU");
        TestView.setObject(modelData, model);
    ...
    public void jsonOutput() throws Exception {
        ...
        if (dataStore.contains(ALL_VAR)) {
            assertEquals("{\"head\":{\"vars\":[\"o\"]},\"results\":{\"bindings\":" +
                    "[{\"o\":{\"type\":\"literal\",\"value\":\"alice\"}}]}}", JsonView.getJsonString(dataStore,
                            outParam));
        }
    }
    ...
    public void orderWithLimit() throws Exception {
        ...
        assertTrue(dataStore.contains(ALL_VAR));
        if (dataStore.contains(ALL_VAR)) {
            assertEquals("{\"head\":{\"vars\":[\"limit_o\"]},\"results\":{\"bindings\":" +
                    "[{\"limit_o\":{\"type\":\"literal\",\"value\":\"alice\"}}]}}", JsonView.getJsonString(dataStore,
                            outParam));
        }
    }
