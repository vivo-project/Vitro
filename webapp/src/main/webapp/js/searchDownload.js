...
$(document).ready(function(){
	...
	$('img#downloadIcon').qtip(
        {
            ...
            content: {
                text:  '<div style="float:right; width:150px;border-left: 1px solid #A6B1B0; padding: 3px 0 0 20px">'
                	...
                	+'<p class ="download-url"><a id=xmlDownload href="' + urlsBase + '/extendedsearch?' + queryText +'&amp;xml=1&amp;hitsPerPage=500">download results in XML format</a></p>'
                	+'<p class ="download-url"><a id=csvDownload href="' + urlsBase + '/extendedsearch?' + queryText +'&amp;csv=1&amp;hitsPerPage=500">download results in CSV format</a></p>'
                	+'</div>'
            },
            position: {
                my: 'top left',
    ...
    $( "#slider-vertical" ).slider({
	      ...
	      slide: function( event, ui ) {
	        $( "#amount" ).val( ui.value );
	        $('#csvDownload').attr("href", urlsBase + '/extendedsearch?' + queryText +'&csv=1&hitsPerPage=' + ui.value);
	        $('#xmlDownload').attr("href", urlsBase + '/extendedsearch?' + queryText +'&xml=1&hitsPerPage=' + ui.value);
	      }
	    });
