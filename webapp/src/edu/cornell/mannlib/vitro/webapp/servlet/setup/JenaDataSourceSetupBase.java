...
public class JenaDataSourceSetupBase extends JenaBaseDaoCon {
   ...
   // The following strings name the Jena models used in the application that
   // are backed by the Jena DB. The models named by these strings along with
   // models that don't have URIs (i.e. the models in files in the submodels
   // directory) are combined into other model configurations (union, submodel...)
   // in JenaPersistentDataSourceSetup and JenaDataSourceSetup and they
   // are stored in the (a?) servlet context.
   
   // TODO: remove this comment when the code is complete.
   //This currently contains Abox plus any TBox axioms that are not in the submodels directories.
   // (that should be TBox local extensions) --
   ...
   // TODO: remove when code done. Currently contains inferences generated by pellet based on the data in kb-2 and the submodels.
