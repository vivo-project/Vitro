...
public class ContextNodeFields implements DocumentModifier{
	...
	private static final List<String> queryList = new ArrayList<String>();
	
	static {
		queryList.add(prefix + 		"SELECT " +
		...
		queryList.add(prefix + 		"SELECT DISTINCT (str(?ContextNodeProperty) as ?contextNodeProperty)  WHERE {" 
				+ "?uri rdf:type foaf:Agent  ; ?b ?c . "
				+ " ?c rdf:type core:Role ; core:roleIn ?Organization ."
				+ " ?Organization rdfs:label ?ContextNodeProperty . "
				+ " } ORDER BY ?ContextNodeProperty ");
		
		queryList.add(prefix + 		"SELECT (str(?ContextNodeProperty) as ?contextNodeProperty) WHERE {" 
				+ "?uri rdf:type foaf:Agent  ; ?b ?c . "
...
//    		targetField.addValue(" " + getSubjectAreaAndResearchAreaOf(individual.getURI()), targetField.getBoost());
    ...
    private Object getPropertiesAssociatedWithContextNodes(String uri) {
	...
	//	fullModel.enterCriticalSection(Lock.READ);
		try{	
			for(String thisQuery : queryList){
				...
				sparqlQuery = QueryFactory.create(thisQuery, Syntax.syntaxARQ);
					...
					try{
						...
						while(results.hasNext()){
							...
							RDFNode contextNodeProperty = soln.get("contextNodeProperty");
							if(contextNodeProperty != null){
								propertyValues.append(" " + contextNodeProperty.toString());
							}else{
								log.debug("contextNodeProperty is null ");
							}
						}
					}catch(Throwable t){
						log.error("Error in getPropertiesAssociatedWithContextNodes() ");
					} finally{
						qExec.close();
					}
			}
