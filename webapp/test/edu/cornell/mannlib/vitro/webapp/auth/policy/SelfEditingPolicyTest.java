...
public class SelfEditingPolicyTest extends AbstractTestClass {
	...
	public void testProhibitedProperties() {
		...
		whatToAuth = new AddDataPropertyStatement(ontModel, SELFEDITOR_URI,
				SAFE_PREDICATE, SOME_LITERAL);
		assertDecision(AUTHORIZED, policy.isAuthorized(ids, whatToAuth));

		whatToAuth = new AddDataPropertyStatement(ontModel, SELFEDITOR_URI,
				UNSAFE_PREDICATE, SOME_LITERAL);
		assertDecision(INCONCLUSIVE, policy.isAuthorized(ids, whatToAuth));
	}
	...
	public void testVisitIdentifierBundleEditDataPropStmt() {
		whatToAuth = new EditDataPropertyStatement(ontModel, SELFEDITOR_URI,SAFE_PREDICATE, SOME_LITERAL);
		assertDecision(AUTHORIZED, policy.isAuthorized(ids, whatToAuth));

		whatToAuth = new EditDataPropertyStatement(ontModel, SELFEDITOR_URI, UNSAFE_PREDICATE, SOME_LITERAL);
		assertDecision(INCONCLUSIVE, policy.isAuthorized(ids, whatToAuth));

		whatToAuth = new EditDataPropertyStatement(ontModel, UNSAFE_RESOURCE, SAFE_PREDICATE, SOME_LITERAL);
		assertDecision(INCONCLUSIVE, policy.isAuthorized(ids, whatToAuth));

		whatToAuth = new EditDataPropertyStatement(ontModel, SAFE_RESOURCE, SAFE_PREDICATE, SOME_LITERAL);
		assertDecision(INCONCLUSIVE, policy.isAuthorized(ids, whatToAuth));
	}
